...
import { UploadCloud, SkipForward, Link2, RefreshCw, AlertTriangle, Loader2 } from "lucide-react";

const PI_UPLOAD_SCHEMA = {
...
// AI-read fields off the uploaded PI file, checked against what's typed
// into the form on this step (and the vendor already selected/chosen).
const PI_DOC_MAPPING = [
  { extractedKey: "vendorName", enteredKey: "vendorName", label: "Vendor name", type: "text" },
  { extractedKey: "documentNumber", enteredKey: "documentNumber", label: "PI number", type: "text" },
...
export default function PIStep({ vendor, purchaseOrder, onUploaded, onSkip, onBack }) {
  ...
  const v = useFormValidation(PI_UPLOAD_SCHEMA);
  ...
  // disagrees is surfaced as a warning below (see PI_DOC_MAPPING above).
  const { status: aiStatus, fields: aiFields } = useAutoExtractOnUpload(
    mode === "upload" ? file : null,
    { documentType: "proformaInvoiceDoc" }
  );
  ...
  const aiMismatches =
    aiStatus === "done"
      ? findMismatches(aiFields, { vendorName: vendor?.companyName, documentNumber, totalQuantity }, PI_DOC_MAPPING)
      : [];
          ...
          <form id="pi-upload-form" onSubmit={handleSubmit} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            {aiMismatches.length > 0 && (
              <div className="sm:col-span-2">
                <DocumentMismatchWarning mismatches={aiMismatches} documentLabel="the uploaded PI" />
              </div>
            )}
          </form>
        )}
